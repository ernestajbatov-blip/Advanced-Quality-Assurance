        ...
        return (
            <div className={styles.CustomTooltip}>
                {payload.map((entry, index) => (
                    <div key={`item-${index}`} className={`${styles.tooltipItem} ${entry.name === "Дебит за предыдущие сутки" ? styles.tooltipRed : styles.tooltipGreen}`}>
        ...
        fetch2Hours()
            .then((response) => {
                ...
                const formattedData = xLabels.map((label, index) => ({
                    name: label,
                    devit_last_day: Math.floor(fetchedData[index]?.debit_last_day || 0),
